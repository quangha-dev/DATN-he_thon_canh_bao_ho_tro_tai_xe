...
export default function Header({ title, description, onMenuClick }: HeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-30 bg-[var(--sf-bg-header)] backdrop-blur-xl border-b border-slate-200/80 dark:border-white/8">
      <div className="flex items-center justify-between px-4 sm:px-6 h-16">
        {/* Left: Title + Description */}
        <div className="flex flex-1 min-w-0 items-center gap-3">
          <button type="button" onClick={onMenuClick} className="lg:hidden rounded-xl border border-slate-200 bg-white p-2 text-slate-600 shadow-sm dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300" aria-label="Mở menu điều hướng">
        ...
        <div className="hidden md:flex flex-1 max-w-lg mx-8">
          <button
            ...
            className="relative w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl bg-white/80 dark:bg-white/[0.04] hover:bg-white dark:hover:bg-white/[0.07] border border-slate-200/80 dark:border-white/8 text-left text-sm text-slate-400 dark:text-slate-500 transition-all cursor-pointer shadow-sm"
          >
